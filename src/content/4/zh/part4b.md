...
我们现在将开始为后端编写测试。因为后端不包含任何复杂的逻辑，没有必要编写[单元测试](https://en.wikipedia.org/wiki/Unit_testing)。我们唯一可能要单元测试的是用于格式化笔记的 _toJSON_ 方法。
...
在某些情况下，通过模拟数据库而不是使用真正的数据库来实现一些后端测试是有益的。可以用来模拟数据库的库有 [mongodb-memory-server](https://github.com/nodkz/mongodb-memory-server)。
...
由于我们的应用的后端仍然相对简单，我们将决定通过其 REST API 测试整个应用，这样也包括数据库。这种将系统的多个组件作为一个整体进行测试的测试，称为[集成测试](https://en.wikipedia.org/wiki/Integration_testing)。
...
在教材的前几章中，我们提到当后端服务端运行在 Fly.io 或 Render 中时，处于<i>生产</i>模式。
...
Node 中的惯例是用 <i>NODE\_ENV</i> 环境变量来定义应用的执行模式。在我们当前的应用中，如果应用<i>不</i>在生产模式下，我们才加载 <i>.env</i> 文件中定义的环境变量。
...
接下来，让我们改变笔记应用中 <i>package.json</i> 的脚本，使得在运行测试时，<i>NODE\_ENV</i> 为 <i>test</i> 值。
...
我们在 _npm run dev_ 脚本中规定应用的模式为 <i>development</i>。我们还规定默认的 _npm start_ 命令将定义模式为 <i>production</i>。
...
我们在脚本中指定应用模式的方式有一个小问题：无法在 Windows 上运行。我们可以通过安装 [cross-env](https://www.npmjs.com/package/cross-env) 包为依赖项来纠正这个问题，用命令：
...
然后我们就可以在 <i>package.json</i> 中定义的 npm 脚本中通过使用 cross-env 库来实现跨平台兼容。
...
现在我们可以修改应用在不同模式下的运行方式。举个例子，我们可以让应用在运行测试时使用一个专门的测试数据库。
...
我们可以在 Mongo DB Atlas 中创建专门的测试数据库。在有很多人开发同一个应用的情况下，这不是最佳的解决方案。测试的执行特别需要同时运行的测试不会使用同一个数据库实例。
...
在安装并运行在开发者本地机器上的数据库上运行测试会更好。最佳的解决方案是让每个执行的测试都使用它专门的数据库。“相对简单”的实现方法有[在内存中运行 Mongo](https://docs.mongodb.com/manual/core/inmemory/) 或使用 [Docker](https://www.docker.com) 容器。我们不打算将事情复杂化，所以我们将继续使用 MongoDB Atlas 数据库。

<!-- Let's make some changes to the module that defines the application's configuration in _utils/config.js_: -->
让我们对定义应用配置的 _utils/config.js_ 文件做一些修改：
...
<i>.env</i> 文件中有<i>分别</i>用于开发和测试的数据库地址的<i>变量</i>。
...
我们实现的 _config_ 模块与 [node-config](https://github.com/lorenwest/node-config) 包略有相似。编写我们自己的实现是合理的，因为我们的应用很简单，同时也因为它给我们带来了宝贵的经验。

<!-- These are the only changes we need to make to our application's code.-->
这些是我们对应用的代码唯一需要进行的修改。
...
你可以在[这个 GitHub 仓库](https://github.com/fullstack-hy2020/part3-notes-backend/tree/part4-2)的 <i>part4-2</i> 分支中找到我们当前应用的全部代码。
...
该测试从 <i>app.js</i> 模块中导入 Express 应用程序，并用 supertest 函数将其封装到一个所谓的 [superagent](https://github.com/visionmedia/superagent) 对象中。该对象赋值给 api 变量，测试可以使用它向后端发送 HTTP 请求。
...
我们的测试向 <i>api/notes</i> url 发送 HTTP GET 请求，并验证请求是否以状态码 200 响应。测试还验证 <i>Content-Type</i> 标头是否设置为 <i>application/json</i>，也就是数据的格式是否是我们想要的。

<!-- Checking the value of the header uses a bit strange looking syntax: -->
检查标头的值使用了一个看起来有点奇怪的语法：
...
期望的值现在是用[正则表达式](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions)，简称 regex 定义的。regex 以斜杠 / 开始和结束，因为期望值的字符串 <i>application/json</i> 也包含相同的斜杠，因此在其之前添加一个 \，这样就不会将其解释为 regex 终止字符。
...
但这里的问题是，如果使用字符串，标头的值必须完全相同。对于我们使用的正则表达式，标头只要<i>包含</i>相关字符串就可以了。标头的实际值为 <i>application/json; charset=utf-8</i>，也就是说，它还包含有关字符编码的信息。但是，我们的测试不关心这个，因此最好将测试定义为正则表达式，而不是确切的字符串。
...
该测试包含一些我们将在[稍后](/zh/part4/测试后端应用#async-await)探讨的细节。定义测试的箭头函数前有 <i>async</i> 关键字，而获取 <i>api</i> 对象的函数调用前有 <i>await</i> 关键字。我们将编写一些测试，然后仔细了解此 async/await 的魔力。现在不必担心它们，只要确保示例测试正确工作即可。 async/await 语法与向 API 发送请求是<i>异步</i>操作的事实有关。async/await 语法可用于像同步代码那样编写异步代码。
...
一旦所有测试（当前只有一个）运行完毕，我们必须关闭 Mongoose 使用的数据库连接。这可以使用 [after](https://nodejs.org/api/test.html#afterfn-options) 方法轻松实现：
...
一个微小但重要的细节：在本部分的[开头](/zh/part4/从后端结构到测试入门#project-structure)，我们将 Express 应用程序提取到 <i>app.js</i> 文件中，而 <i>index.js</i> 文件的作用已更改为通过 _app.listen_ 在指定端口启动应用程序：
...
测试只使用 <i>app.js</i> 文件中定义的 Express 应用。
...
这两个测试都将请求的响应存储在 _response_ 变量中，并且与使用 _supertest_ 提供的方法来验证状态码和标头的前一个测试不同，这次我们正在检查存储在 <i>response.body</i> 属性中的响应数据。我们的测试使用 assert-library 的 [strictEqual](https://nodejs.org/docs/latest/api/assert.html#assertstrictequalactual-expected-message) 方法验证响应数据的格式和内容。
...
我们目前测试的问题是，测试的成功依赖于数据库的状态。如果测试的数据库正好有两条笔记，其中一条的内容是 <i>'HTML is easy'</i>，那么测试才会通过。为了使我们的测试更加稳健，我们必须在运行测试之前以一种可控的方式重置数据库并生成所需的测试数据。
...
在我们编写更多的测试之前，让我们先来看一下 _async_ 和 _await_ 关键字。
...
ES7 中引入的 async/await 语法可以以一种让代码看起来是同步的方式来使用<i>返回 Promise 的异步函数</i>。

<!-- As an example, the fetching of notes from the database with promises looks like this:-->
举个例子，用 Promise 从数据库中获取笔记的过程看起来是这样的：
...
_Note.find()_ 方法返回一个 Promise ，我们可以通过用 _then_ 方法注册一个回调函数来访问操作的结果。
...
then 链是好的，但我们可以做得更好。ES6中引入的[生成器函数](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Generator)提供了一种[聪明的方法](https://github.com/getify/You-Dont-Know-JS/blob/1st-ed/async%20%26%20performance/ch4.md#iterating-generators-asynchronously)，以“看起来是同步的”方式编写异步代码。该语法有点笨重，没有得到广泛使用。
...
这段代码看起来和同步代码完全一样。代码在 <em>const notes = await Note.find({})</em> 处暂停执行，等到相关的 Promise 被<i>满足</i>，才继续执行到下一行。当执行继续时，Promise 返回的操作结果被赋值给 _notes_ 变量。
...
由于目前所有的异步操作都是在一个函数内完成的，所以只需将路由处理函数改为 async 函数即可。获取所有笔记的路由
...
你可以在[这个 GitHub 仓库](https://github.com/fullstack-hy2020/part3-notes-backend/tree/part4-3)的<i>part4-3</i>分支中找到我们当前应用的全部代码。
...
当重构代码，总是有[倒退](https://en.wikipedia.org/wiki/Regression_testing)的风险，这意味着现有的功能可能被破坏。让我们先为 API 的每条路由写一个测试，来重构剩下的操作。
...
测试失败了，因为当创建一个新的笔记时，我们意外地返回状态码 <i>200 OK</i>。让我们把它改为 <i>201 CREATED</i>：
...
你需要在处理函数的开头添加 _async_ 关键字来启用 _async/await_ 语法。代码变得更简单了。
...
值得注意的是，我们不再需要专门处理转发错误了。在使用 Promise 的代码中，我们需要像这样将可能出现的错误传递给中间件：
...
当使用 _asnyc/await_ 语法时，如果 await 语句抛出错误或 await 的 Promise 被拒绝，Express 会[自动调用](https://expressjs.com/en/guide/error-handling.html)错误处理中间件。这使得最终代码更清爽。
...
**注意：**这个功能是从 Express 5 开始有的。如果你在 2025 年 3 月 31 日之前将 Express 安装为依赖项的话，你可能用的还是 4。你可以在 _package.json_ 文件中查看你项目的 Express 版本，如果你的版本是旧的，可以用下列命令升级到 5：
...
使用 SuperTest 库编写一个测试，向 <i>/api/blogs</i> URL 发送 HTTP GET 请求。验证博客列表应用程序以 JSON 格式返回正确数量的博客文章。
...
请注意，你必须对代码进行类似的更改，这些更改在 [材料](/zh/part4/测试后端应用#test-environment) 中进行了，例如定义测试环境以便你可以编写使用单独数据库的测试。
...
**注意：**在编写测试时**<i>最好不要全部执行</i>**，只执行你正在处理的测试。[此处](/zh/part4/测试后端应用#test-environment) 了解更多相关信息。
...
编写一个测试来验证向 <i>/api/blogs</i> URL 发送 HTTP POST 请求可以成功创建新的博客文章。至少验证系统中的博客总数增加了 1。你还可以验证博客文章的内容是否正确保存到数据库中。
...
编写与通过 <i>/api/blogs</i> 端点创建新博客相关的测试，验证如果请求数据中缺少 <i>title</i> 或 <i>url</i> 属性，后端将以状态码 <i>400 Bad Request</i> 响应请求。
